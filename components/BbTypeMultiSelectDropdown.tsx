...
import { BBType } from '../types';
import { XMarkIcon } from './icons/XMarkIcon';

interface BbTypeMultiSelectDropdownProps {
  options: BBType[];
  selectedOptions: BBType[];
  onSelectionChange: (newSelected: BBType[]) => void;
  onClose: () => void;
  position: { top: number; left: number };
}

export const BbTypeMultiSelectDropdown: React.FC<BbTypeMultiSelectDropdownProps> = ({
  options,
  selectedOptions,
  onSelectionChange,
  onClose,
  ...
  const handleCheckboxChange = (option: BBType) => {
  ...
  return (
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-labelledby="bbtype-filter-title"
    >
      <div className="flex justify-between items-center p-3 border-b border-gray-200">
        <h3 id="bbtype-filter-title" className="text-sm font-semibold text-gray-700">Filter by BB Type</h3>
        <button
            onClick={onClose}
